...
export const SizeMatrix: React.FC<SizeMatrixProps> = ({ onClose }) => {
  ...
  const [locationFilter, setLocationFilter] = useState<'Capital' | 'Interior' | 'Ambos'>('Ambos');
  const [isFilterModalOpen, setIsFilterModalOpen] = useState(false);
  const [tempBatch, setTempBatch] = useState<number | 'Geral' | null>(null);
  ...
  const batchDataResult = useMemo(() => {
    ...
    orders
      .filter(o => (selectedBatch === 'Geral' || (o.lote || 1) === selectedBatch) && (locationFilter === 'Ambos' || o.local === locationFilter))
      .forEach(order => {
        COLORS.forEach(color => {
  ...
  }, [orders, selectedBatch, locationFilter]);

  const contributors = useMemo(() => {
    if (!selectedCell || selectedBatch === null || selectedBatch === 'Geral') return [];
    
    return orders
      .filter(o => (o.lote || 1) === selectedBatch && (locationFilter === 'Ambos' || o.local === locationFilter))
      .map(o => {
  ...
  }, [selectedCell, orders, selectedBatch, locationFilter]);

  const handleDownloadPDF = () => {
    if (selectedBatch === null) return;
    const reportOrders = orders.filter(o => 
      (selectedBatch === 'Geral' || (o.lote || 1) === selectedBatch) && 
      (locationFilter === 'Ambos' || o.local === locationFilter)
    );
    generateSizeMatrixPDF(reportOrders, unitPrice, stats, selectedBatch, reportComment, locationFilter);
  ...
  const { data, grandTotal, totalVerde, totalTerracota } = batchDataResult || { data: {}, grandTotal: 0, totalVerde: 0, totalTerracota: 0 };
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20 px-2 sm:px-4">
        ...
        <>
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              >
                  <h3 className="text-xl sm:text-2xl font-black text-text-primary uppercase tracking-tight">
                    ...
                    {locationFilter !== 'Ambos' && (
                      <span className="ml-2 text-primary">({locationFilter})</span>
                    )}
                  </h3>
          ...
          <div className="space-y-8 sm:space-y-12">
              {CATEGORIES.map(category => {
                const categoryHasData = data[category] && COLORS.some(color => data[category][color].subTotal > 0);
                  ...
                  <div key={category} className="space-y-4 sm:space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
                      {COLORS.map(color => {
                        ...
                        if (!colorData || colorData.subTotal === 0) return null;
      ...
      <Modal isOpen={isCommentModalOpen} onClose={() => setIsCommentModalOpen(false)} title="Observações do PDF">
        <div className="space-y-4 sm:space-y-6">
          <p className="text-[10px] sm:text-[11px] text-text-secondary font-bold uppercase tracking-widest text-center">
            ...
            {locationFilter !== 'Ambos' && <span className="block text-primary">Filtro aplicado: {locationFilter}</span>}
          </p>
          <TextArea 
      ...
      <Modal 
        ...
        title="Filtro de Produção"
      >
        <div className="space-y-6">
          <p className="text-sm text-text-secondary font-medium text-center">
            Quais pedidos deseja considerar no cálculo da matriz?
          </p>
          
          <div className="space-y-3">
            {(['Capital', 'Interior', 'Ambos'] as const).map((option) => (
              <label 
                key={option}
              ...
              >
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  locationFilter === option ? 'border-primary' : 'border-text-secondary/30'
                }`}>
                  {locationFilter === option && <div className="w-2.5 h-2.5 rounded-full bg-primary"></div>}
                </div>
                <span className={`font-bold uppercase tracking-widest text-xs ${
                  locationFilter === option ? 'text-primary' : 'text-text-secondary'
                }`}>
                  {option}
                </span>
              </label>
