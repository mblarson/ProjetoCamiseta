...
export const SplashScreen: React.FC<{ onAccess: () => void, loading: boolean }> = ({ onAccess, loading }) => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] bg-splash-bg flex items-center justify-center overflow-hidden">
      {/* Decorative Shapes */}
      <div className="absolute top-[-20%] left-[-15%] w-72 h-72 bg-white/5 rounded-full filter blur-xl"></div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 w-full px-8 mt-auto mb-20 animate-in fade-in slide-in-from-bottom-10 duration-1000">
        <div className="text-center w-full max-w-lg mb-8">
          <h1 className="text-6xl font-black mb-4 tracking-tighter leading-none text-white">
            Evolução do Cuidado
          </h1>
          <p className="text-white/60 text-lg font-light tracking-tight max-w-sm mx-auto">
        ...
        <button 
          onClick={handleClick} 
          disabled={clicked}
          className="w-full max-w-sm h-16 bg-white text-splash-bg font-black text-sm tracking-widest rounded-full shadow-[0_10px_40px_rgba(255,255,255,0.2)] transition-all flex items-center justify-center gap-3 uppercase active:scale-95 hover:shadow-2xl"
        >
          {clicked ? (
