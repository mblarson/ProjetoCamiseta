...
export const HomeMenu: React.FC<{ onNavigate: (s: Section) => void, isOrdersOpen: boolean }> = ({ onNavigate, isOrdersOpen }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] gap-8 md:gap-16 animate-in fade-in zoom-in-95 duration-1000 pt-8 md:pt-24 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full max-w-[1400px]">
        {isOrdersOpen ? (
          <HomeCard 
            icon="fa-cart-plus" 
            title="Novo Pedido" 
...
const HomeCard: React.FC<{ icon: string, title: string, onClick: () => void }> = ({ icon, title, onClick }) => (
  <MovingBorderButton
    ...
    className="bg-slate-50 p-4 md:p-8 text-left justify-start gap-4 md:gap-6 border-transparent backdrop-blur-none"
  >
